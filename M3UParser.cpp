...
status_t M3UParser::parse(const void *_data, size_t size) {
    ...
    bool hasParsed_MEDIA_SEQUENCE = false;
#endif
    while (offset < size) {
        ...
        AString line;
//letv media add start
#if 1
        int i;
        for (i = 0; i <offsetLF; i++){
            if (data[offsetLF -1 -i] != '\r')
                break;
        }
        line.setTo(&data[offset], offsetLF - offset -i);
#else
...
//letv media add end
        ...
        if (mIsExtM3U) {
            ...
            if (line.startsWith("#EXT-X-TARGETDURATION")) {
...
#ifdef LETV_HLS_OPTIMIZATION
                if(!hasParsed_MEDIA_SEQUENCE){
                    err = parseMetaData(line, &mMeta, "media-sequence");
                    hasParsed_MEDIA_SEQUENCE = true;
                }
#else
                err = parseMetaData(line, &mMeta, "media-sequence");
#endif
                ...
                err = parseProgramDatetime(line,&itemMeta);
...
status_t M3UParser::parseProgramDatetime(
        const AString &line, sp<AMessage> *meta){
     ssize_t colonPos = line.find(":");
     int time;
    ...
    if (meta->get() == NULL) {
     ...
     (*meta)->setString("program-time", line.c_str() + colonPos + 1);
     ALOGE("program-time is %s",line.c_str() + colonPos + 1);
